...
public class ViewPendingSubscriptions extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException, ServletException, IOException {
        ...
        try {
            if(user.getRole().equals("pt")){
                ...
                if(specific_search.equals("all")){
                    members = dao_factory.getMemberDAO().getAllStatePendingMembersOrderedByName(user.getState_id());
                }else{
                    ...
                    members = dao_factory.getMemberDAO().getDepartmentPendingMembersOrderedByName(state_id, city_id, user.getRole());
                ...
                members = dao_factory.getMemberDAO().getDepartmentPendingMembersOrderedByName(user.getState_id(), user.getCity_id(), user.getRole());
            }else if(user.getRole().equals("kd") || user.getRole().equals("su")){
                departments = dao_factory.getDepartmentDAO().getAllDepartments();
                
                if(specific_search.equals("all")){
                    members = dao_factory.getMemberDAO().getAllPendingMembersOrderedByName();
                }else{
                    String[] specific_search_parts = specific_search.split("\\.");
                    String state_id="";
                    String city_id="";
        ...
        request.setAttribute("pending", "yes");
